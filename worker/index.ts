...
// Inject per-scheme <title>/description/Open Graph/Twitter meta into index.html so
// shared links and crawlers get the scheme name, an evocative description, and the
// palette OG image. The SPA still hydrates and takes over on top of this.
async function ssr(request: Request, env: Env, url: URL): Promise<Response> {
  try {
